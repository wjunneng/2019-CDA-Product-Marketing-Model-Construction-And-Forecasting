...
class CatBoost(object):
    ...
    def cv(self, **params):
        ...
        for model_seed in range(num_model_seed):
            ...
            prediction_lgb = np.zeros((self.X_test.shape[0]))

            # 存放特征重要性
            feature_importance_df = pd.DataFrame()
            for index, value in enumerate(DefaultConfig.before_after):
                print('第 ' + str(value) + ' 折')
                ...
                gc.collect()
                bst = cbt.CatBoostClassifier(iterations=2019, learning_rate=0.005, verbose=300,
                                             early_stopping_rounds=1666,
                                             task_type='GPU', use_best_model=True, random_state=seeds[num_model_seed])
                bst.fit(train_x, train_y, eval_set=(test_x, test_y))

                oof_lgb[test_x.index] += bst.predict_proba(test_x)[:, 1]
                prediction_lgb += bst.predict_proba(self.X_test.drop(DefaultConfig.label_column, axis=1))[:, 1] / len(
                    DefaultConfig.before_after)
                gc.collect()
        ...
        if feature_importance is not None:
            ...
            # 按照flod分组
            group = feature_importance.groupby(by=['fold'])
    ...
    def main(self, **params):
        ...
        for model_seed in range(num_model_seed):
            ...
            prediction_lgb = bst.predict_proba(self.X_test.drop(DefaultConfig.label_column, axis=1))[:, 1]
